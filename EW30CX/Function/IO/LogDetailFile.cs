...
namespace EW30CX.Function.IO {
    public class LogDetailFile {
        ...
        public void createLog() {
            ...
            //create log system
            string file_log_system = string.Format("EW30CX_{0}_{1}_{2}_system.txt", mac, DateTime.Now.ToString("HHmmss"), myGlobal.myTesting.totalResult);
            using (var sw = new StreamWriter(System.IO.Path.Combine(logdir, file_log_system), true, Encoding.Unicode)) {
                sw.WriteLine("Product: EW30CX");
                sw.WriteLine(log_data);
                sw.WriteLine(myGlobal.myTesting.logSystem);
            }
            
            //create log uart
            string file_log_uart = string.Format("EW30CX_{0}_{1}_{2}_uart.txt", mac, DateTime.Now.ToString("HHmmss"), myGlobal.myTesting.totalResult);
