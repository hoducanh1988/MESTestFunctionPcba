...
namespace EW12SG.Function.IO {
    public class LogDetailFile {
        ...
        public LogDetailFile() {
            ...
            logdir = string.Format("{0}\\EW12SG", logdir);
        ...
        public void createLog() {
            //create log system
            string file_log_system = string.Format("EW12SG_{0}_{1}_{2}_system.txt", mac, DateTime.Now.ToString("HHmmss"), myGlobal.myTesting.totalResult);
            File.WriteAllText(System.IO.Path.Combine(logdir, file_log_system), "Product: EW12SG\r\n" + myGlobal.myTesting.logSystem, Encoding.Unicode);

            //create log uart
            string file_log_uart = string.Format("EW12SG_{0}_{1}_{2}_uart.txt", mac, DateTime.Now.ToString("HHmmss"), myGlobal.myTesting.totalResult);
            File.WriteAllText(System.IO.Path.Combine(logdir, file_log_uart), "Product: EW12SG\r\n" + myGlobal.myTesting.logUart, Encoding.Unicode);
        }
